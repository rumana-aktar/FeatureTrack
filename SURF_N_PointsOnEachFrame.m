...
% largest N tracks on each frame in different colors
...
files = dir(fullfile(imgDir,sprintf('%s*.%s','Data','jpg')));
%--output location
outDir=strcat(rootDir, 'Shot', num2str(shot), '_Points_1000_new/');mkdir(outDir);
...
while(i<=shot_end)
    ...
    bluePoints=[]; greenPoints=[]; redPoints=[]; grayPoints=[]; edgeColor=[1 0 0];%--go red
    ...
    for xx=row_id' % row_id contains row_ids of matches of desired track no. for example, if frame1 has 10 tracks, then it tells which 10 rows of 'file' is of interest
        ...
        if start_frame==i && end_frame==i
            color=[0 0 1]; 
            bluePoints=[bluePoints; [colIdx rowIdx]];
            %--go blue, this point starts and ends at the same frame
        elseif start_frame==i %|| start_frame==i-1 || start_frame==i-2 || start_frame==i-3 || start_frame==i-4
            color=[0 1 0];
            greenPoints=[greenPoints; [colIdx rowIdx]];
        ...
        elseif end_frame==i %|| end_frame==i+1 || end_frame==i+2 || end_frame==i+3 || end_frame==i+4
            color=[.8 .8 .8];
            grayPoints=[grayPoints; [colIdx rowIdx]];
            %--go gray. this is a soon to be removed frame, will be removed in less than 5 frames        
        else
            color=[1 0 0]; 
            redPoints=[redPoints; [colIdx rowIdx]];
            %--go red, consistent feature point, alive longer than 5 frames
        end
        %boxColor=[1 1 1];%--go white
    end  
    if size(bluePoints,1)>0; scatter(bluePoints(:, 1), bluePoints(:,2), 'o', 'MarkerFaceColor', [0 0 1],'MarkerEdgeColor', edgeColor); end;
    if size(greenPoints,1)>0; scatter(greenPoints(:, 1), greenPoints(:,2), 'o', 'MarkerFaceColor', [0 1 0],'MarkerEdgeColor', edgeColor); end;
    if size(redPoints,1)>0; scatter(redPoints(:, 1), redPoints(:,2), 'o', 'MarkerFaceColor', [1 0 0],'MarkerEdgeColor', edgeColor); end;
    if size(grayPoints,1)>0; scatter(grayPoints(:, 1), grayPoints(:,2), 'o', 'MarkerFaceColor', [.8 .8 .8],'MarkerEdgeColor', edgeColor); end;
