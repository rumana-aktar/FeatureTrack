...
load('/Volumes/E/ABQ_all/ABQ_215_vmz/Files/BA/shot2/2_points.mat');

frameIDs=unique(BA(:, 1));
for fr=frameIDs'
    fr
    indexFr=find(BA(:,1)'==fr);
    dataFr=BA(indexFr, :);
    trackIDs=dataFr(indexFr, 4);
    
    frID_trID=dataFr(:, [1 4]);
    uniqueTrackId=unique(trackIDs);
    [u,I,J] = unique(frID_trID, 'rows', 'first')
    hasDuplicates = size(u,1) < size(frID_trID,1)
...
%--get image directory
imgDir='/Volumes/E/Oxford_dataset/bikes/';
files = dir(fullfile(imgDir,sprintf('%s*.%s','img','ppm')));
%--output location
...
while(i<=shot_end)
    ...
    if ismember(i, RefList)
      I = insertText(uint8(I), [720 1+2*vGap ], 'REF', 'AnchorPoint', 'RightTop', 'fontSize', fontSize, 'BoxColor', 'red', 'TextColor', 'white'); %1800      
    elseif ismember(i, WRefList) %--show if it is a WGReference frame  
      I = insertText(uint8(I), [720 1+2*vGap ], 'WREF', 'AnchorPoint', 'RightTop', 'fontSize', fontSize); %1800      
    end
